...
public class RestaurantOrder implements Order // Заказы в самом ресторане
{
    ...
    public boolean add(MenuItem item) // Метод добавления позиции в заказ, принимающий в качестве параметра элемент еды
    {
        counter++;
    ...
    public MenuItem[] GetItem() // Метод возвращает массив всех позиций в данном заказе
    {
        try // Если size отрицательна, мы не можем дальше продолжать работу
        {
            int index = 0;
            MenuItem[] items = new MenuItem[counter];
            ListNode current = head;
    ...
    {
        float summ = 0;
        for (MenuItem index : GetItem()) // Перебор в массиве по всем позициям в заказе
    ...
    {
        ListNode current = head;
        MenuItem[] items = GetItem(); // Создаём сырой массив, размером с количество заказанных позиций в заказе
        int index = 0;
        boolean flag;
    ...
    public MenuItem[] sorted_totalItem() // Метод, возвращающий массив позиций в данном заказе, отсортированный по убыванию цены
    {
        MenuItem[] items = GetItem(); // Создание массива с помощью вызова метода GetItem
        MenuItem temp; // Временная переменная, служащая для хранения некоторых элементов в массиве для сортировки
